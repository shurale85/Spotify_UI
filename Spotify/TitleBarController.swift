...
class TitleBarController: UIViewController {
    ...
    func setupNavBar() {
        ...
        //to remove shadow under nav bar - does not work yet
        let img = UIImage()
    ...
    func makeBarButtonItem(text: String, selector: Selector) -> UIBarButtonItem {
        ...
        let attributes = [
            ...
            NSAttributedString.Key.foregroundColor: UIColor.label //color will be changed by system acccording to mode
        ]
    ...
    @objc func musicTapped() {
        ...
        animateTrasintion(fromVC: viewControllers[1], toVC: viewControllers[0]) { _ in
    ...
    @objc func podcastTapped() {
        ...
        animateTrasintion(fromVC: viewControllers[0], toVC: viewControllers[1]) { _ in
    ...
    func animateTrasintion(fromVC: UIViewController, toVC: UIViewController, completion: @escaping ((Bool) -> Void)) {
        guard
